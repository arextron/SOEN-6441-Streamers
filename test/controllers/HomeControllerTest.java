...
public class HomeControllerTest extends WithApplication {
    ...
    public void testIndex() {
        ...
        // Call the index method in HomeController
    ...
    public void testSearchWithNullQuery() throws Exception {
        ...
        CompletionStage<Result> resultStage = controller.search(null, request);
        Result result = resultStage.toCompletableFuture().get();

        assertEquals(OK, result.status()); // Ensure the response status is OK
        assertEquals("Please provide a search query.", contentAsString(result)); // Check the response content
    }
    ...
    @Test
    public void testWordStats_NullOrEmptyQuery() throws Exception {
        // Test with a null query
        CompletionStage<Result> nullResultStage = controller.wordStats(null);
        Result nullResult = nullResultStage.toCompletableFuture().get();
        assertEquals(BAD_REQUEST, nullResult.status());
        assertEquals("Please provide a search query.", contentAsString(nullResult));

        // Test with an empty query
        CompletionStage<Result> emptyResultStage = controller.wordStats("   ");
        Result emptyResult = emptyResultStage.toCompletableFuture().get();
        assertEquals(BAD_REQUEST, emptyResult.status());
        assertEquals("Please provide a search query.", contentAsString(emptyResult));
    }
    ...
    @Test
    public void testSetVideoCache() throws Exception {
        ...
        // Prepare a map to simulate a video cache
        Map<String, List<VideoResult>> newVideoCache = new HashMap<>();
        newVideoCache.put("testQuery", mockVideos);

        // Call the setter
        controller.setVideoCache(newVideoCache);

        // Validate the cache was updated
        assertEquals(newVideoCache, controller.videoCache);
        assertTrue(controller.videoCache.containsKey("testQuery"));
        assertEquals(mockVideos, controller.videoCache.get("testQuery"));
    }
